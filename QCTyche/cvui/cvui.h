...
*/
template <typename T>
bool trackbar(cv::Mat& theWhere, int theX, int theY, int theWidth, T* theValue, T theMin, T theMax, int theSegments = 1,
...
bool trackbar(int theWidth, T* theValue, T theMin, T theMax, int theSegments = 1, const char* theLabelFormat = "%.1Lf",
...
template <typename num_type>
bool trackbar(cv::Mat& theWhere, int theX, int theY, int theWidth, num_type* theValue, num_type theMin, num_type theMax,
...
namespace internal {
	...
	int counter(cvui_block_t& theBlock, int theX, int theY, int* theValue, int theStep, const char* theFormat) {
		...
		if (internal::button(theBlock, theX, theY, 22, 22, "-", false))     *theValue -= theStep;
...
namespace render {
	...
	void trackbar(cvui_block_t& theBlock, int theState, cv::Rect& theShape, double theValue,
		...
		if (aShowSteps && !aHideAllLabels)  trackbarSteps(theBlock, theState, theShape, theValue, theParams, aWorkingArea);

		if (!aHideAllLabels)                trackbarSegments(theBlock, theState, theShape, theValue, theParams, aWorkingArea);
...
void update(const cv::String& theWindowName) {
	...
	if (internal::gDelayWaitKey > 0)    internal::gLastKeyPressed = cv::waitKey(internal::gDelayWaitKey);
